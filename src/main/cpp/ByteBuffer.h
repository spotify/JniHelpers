...
/**
 * @brief Class for exchanging Java byte[] data with a ByteBuffer object
 */
class EXPORT ByteBuffer {
public:
  /**
   ...
   * in ByteBuffer instances.
   */
  ByteBuffer(void *data, const size_t numBytes, bool copyData);
   ...
   */
  ByteBuffer(JNIEnv *env, jbyte data);
   ...
   */
  JniLocalRef<jobject> toJavaByteBuffer(JNIEnv *env) const;

  /**
   * @brief Return a pointer to the data stored by this instance
   *
   * When an instance of ByteBuffer is destroyed, it will attempt to free
   ...
   * @param copyData True if the data should be copied to this instance
   *
   * If copyData is true, then this ByteBuffer instance owns that data and
   * the original data passed to this method can be freed without worry.
   * However, if copyData is false, then this ByteBuffer effectively just
   * points to that instance instead. In either case, after setting data
   * in a ByteBuffer, it effectively owns that data.
   *
   * When this ByteBuffer is destroyed, it will free the data stored in it,
   ...
   * situation by removing the pointer to the data so the ByteBuffer will
   ...
   */
  void set(JNIEnv *env, jByteBuffer data);
