...
ByteBuffer::ByteBuffer() : _data(NULL), _num_bytes(0) {}

ByteBuffer::ByteBuffer(void *data, const size_t numBytes, bool copyData) :
...
ByteBuffer::ByteBuffer(JNIEnv *env, jobject data) :
_data(NULL), _num_bytes(0) {
  set(env, data);
}
...
JniLocalRef<jByteBuffer> ByteBuffer::toJavaByteBuffer(JNIEnv *env) const {
  JniLocalRef<jByteBuffer> result = env->NewByteBuffer((jsize)_num_bytes);
  JavaExceptionUtils::checkException(env);
  if (_num_bytes == 0 || _data == NULL) {
    return result;
  }
  env->SetByteBufferRegion(result, 0, (jsize)_num_bytes, (jbyte *)_data);
  return result.leak();
}

void ByteBuffer::set(void *data, const size_t numBytes, bool copyData) {
  if (data == NULL && numBytes > 0) {
...
void ByteBuffer::set(JNIEnv *env, jByteBuffer data) {
  if (_data != NULL) {
    free(_data);
  }
  ...
  if (data != NULL) {
    ...
    if (_num_bytes == 0) {
      ...
      env->GetByteBufferRegion(data, 0, (jsize)_num_bytes, (jbyte *)_data);
    }
  }
}
